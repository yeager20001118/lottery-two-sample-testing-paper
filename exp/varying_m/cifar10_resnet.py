"""
CIFAR-10 (ResNet-18) experiment: Varying query size M with fixed reference size N=100.
Reproduces Table 2 (right, CIFAR10-RES18) in the paper.
"""
...
parser = argparse.ArgumentParser(description="CIFAR10-ResNet18: Varying M (query size)")
parser.add_argument('--check', default=1, type=int, help='1=test power, 0=type I error')
parser.add_argument('--N_ref', default=100, type=int, help='Reference sample size N')
parser.add_argument('--N_query', default=[2, 4, 6, 8, 10, 12, 14], nargs='+', type=int,
...
for M in args.N_query:
    print(f"\n{'='*60}")
    print(f"CIFAR10-{args.model_arch} | N={args.N_ref}, M={M}, check={args.check}")
    print(f"{'='*60}")

    results = np.zeros(args.n_exp)
    for kk in range(args.n_exp):
        rs = kk * 1000 + args.seed + M
        H = run_lott('cifar10', args.N_ref, M, rs, args.check,
    ...
    fname = f"cifar10_{args.model_arch}_N{args.N_ref}_M{M}_seed{args.seed}"
